    ...
    public:
        ...
        bool hit(const ray& r, interval ray_t) const {
            ...
            for (int axis = 0; axis < 3; axis++){
                ...
                float t0 = (ax_interval.min - ray_origin[axis]) * (1/ray_direction[axis]);
                float t1 = (ax_interval.max - ray_origin[axis]) * (1/ray_direction[axis]);

                ray_t.min = fmin(ray_t.min,fmin(t0, t1));
                ray_t.max = fmax(ray_t.max, fmax(t0, t1));
